...
function FillPoll() {
  ...
  return (
    <div>
      <Head>
        <title>SamakiZetu | Settings</title>
      </Head>
      <Sidebar />
      <div className="lg:ml-80 ml-16 mr-2 mt-5 p-4 rounded">
        <div className="xl:px-4 w-full">
          <Card className="rounded-b-none">
            <span className="text-3xl">Find your Fish</span>
          </Card>
          <Card>
            <Input
              placeholder="Paste a Tracker ID here...(You can find it in the Tracker history)"
          ...
          {pollID !== "" && poll && (
            <>
              {/* <div className="shadow-2xl bg-primary rounded-lg -mb-8 text-white text-large p-12">
                                <h4 className="text-4xl font-bold text-center">{poll.name} {poll.voteed && `- ${poll.votes} votes`}</h4>
                            </div> */}
              <Card className="break-words mx-10">
                <div className="mb-4 bg-primary rounded-lg text-white text-large p-5">
                  <h4 className="text-4xl font-bold text-center">
                    {poll.name} {poll.voteed && `- ${poll.votes} ordered`}
                  </h4>
                </div>
                      ...
                      {poll.options.map((option, i) => (
                        ...
                        >
                          {({ checked }) => (
                            ...
                            >
                              {option.name} {poll.voteed && `(${option.votes})`}
                            </Pill>
                          )}
                        </RadioGroup.Option>
                      ))}
