...
contract Voting {
    ...
    function vote(uint poll_id, uint option_id) payable public {
        ...
        require(poll_id < polls_created, "Tracker does not exist");
        ...
        // Check that user has not voteed in the future
        require(polls[poll_id].start_time <= block.timestamp, "User has tried Tracking in the future");
        // Check that user has not voteed in the past
        require(polls[poll_id].end_time >= block.timestamp, "User has tried Tracking for the past");
